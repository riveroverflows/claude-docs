...
- ✅ 목적/범위/가정/제외를 별도 소섹션으로 분리 (1.1~1.6)
- ✅ 가정과 제약(`1.4`)과 범위 제외(`1.5`)를 명확히 구분
- ✅ 용어 정의는 인라인 설명 대신 별도 문서(`00-ubiquitous-language.md`) 참조

### 구체성

- ✅ "재고가 충분하다" (X) → "주문 수량 ≤ 재고 수량" (O)
- ✅ "유효한 사용자" (X) → "Customer 식별이 가능해야 한다" (O)
- ✅ "삭제된 브랜드/상품" — 소프트 딜리트 기반 표현, DB 구현 용어 배제
...
- ✅ 서비스 규모 수치를 근거로 성능 목표 수치 제시 (p95 기준)
- ✅ 규모 수치가 기술적 의사결정의 기준점임을 명시

### Scope 명확성

- ✅ `1.5 범위 제외`에서 제외 항목 + 근거/대안 명시
- ✅ 범위 고정 문장 포함 (`1.5` 마지막 줄)
